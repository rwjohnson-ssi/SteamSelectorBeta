#!/usr/bin/env python3
"""Fail when a locally referenced CSS or JavaScript file is missing.
...
def inspect_js_file(path: Path) -> list[tuple[Path, str]]:
    ...
    # as literal strings to helpers such as appendScript().
...
def main() -> int:
    ...
    references.extend(
        ...
        if ".git" not in js_file.parts and ".github" not in js_file.parts
